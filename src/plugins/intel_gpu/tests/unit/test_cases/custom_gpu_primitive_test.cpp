...
// Impl-level contract: custom_gpu_primitive_impl enqueues a real OpenCL kernel, so it
// must not report itself as a CPU implementation. If it does, primitive_inst's allocator
// places the op's own output in lockable host memory, and -- because
// requires_lockable_input() defaults to is_cpu() and has_any_cpu_user_not_shape_of()
// recurses through can_be_optimized() nodes -- every producer feeding it as well. The
// kernel then reads and writes host memory across PCIe.
TEST(custom_gpu_primitive_f32, impl_is_not_cpu_and_producer_stays_in_device_memory) {
    ...
    EXPECT_FALSE(custom_inst->get_impl()->is_cpu())
        << "custom_gpu_primitive_impl runs an OpenCL kernel on the GPU and must not be "
           "classified as a CPU implementation.";
    ...
    if (engine.supports_allocation(allocation_type::usm_device)) {
        ...
        EXPECT_NE(producer_inst->output_memory_ptr()->get_allocation_type(),
            ...
            << "Producer output was forced into usm_host by its custom-op user.";
    }
}
